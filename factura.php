 ...
 * Accesible por administrador, emprendedor (solo sus productos) y por el
 * cliente dueño del pedido (solo su propia factura).
 */
session_start();
include("conexion.php");

if (!isset($_SESSION['usuario_id']) || !in_array($_SESSION['rol'], ['administrador', 'emprendedor', 'cliente'], true)) {
...
// Un cliente solo puede ver la factura de SU PROPIO pedido
if ($es_cliente && (int) $pedido['usuario_id'] !== $usuario_sesion_id) {
...
if (!$es_admin && !$es_cliente && empty($items)) {
...
<div class="acciones">
    <button class="btn" onclick="window.print()">🖨️ Descargar / Imprimir factura</button>
    <a href="<?= $es_admin ? 'admin.php' : ($es_cliente ? 'mis_pedidos.php' : 'emprendedor.php') ?>" class="btn btn-outline">← Volver</a>
</div>
<?php if ($es_cliente): ?>
<p style="max-width:720px;margin:0 auto 16px;text-align:right;font-size:12px;color:var(--muted)">
    Tip: en la ventana de impresión, elige "Guardar como PDF" como destino para descargarla.
</p>
<?php endif; ?>
